...
import org.springframework.samples.petclinic.pet.application.PetBoundary;
import org.springframework.samples.petclinic.pet.application.VisitBoundary;
...
@Controller
 class VisitController {
	...
	private final VisitBoundary visitBoundary;

	private final PetBoundary petBoundary;

	private final OwnerBoundary ownerBoundary;

	public VisitController(VisitBoundary visitBoundary, PetBoundary petBoundary, OwnerBoundary ownerBoundary) {
		this.visitBoundary = visitBoundary;
		this.petBoundary = petBoundary;
	...
	public String initNewVisitForm(@PathVariable Integer ownerId, @PathVariable Integer petId, ModelMap model) {
		...
		model.put("mode", "new");
		model.put("owner", ownerBoundary.findById(ownerId));
		model.put("pet", petBoundary.findWithVisitsById(petId));
